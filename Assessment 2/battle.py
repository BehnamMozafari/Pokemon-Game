...
class Battle:
    ...
    def set_mode_battle(self) -> str:
        ...
        while self.team1.team.__len__() >= 1 and self.team2.team.__len__() >= 1:
            u1 = self.team1.team.pop()
            u2 = self.team2.team.pop()
            if u1.get_speed() > u2.get_speed():
                u2.damage_after_attacked(u1)
                if u2.get_hp() > 0:
                    u1.damage_after_attacked(u2)
                    if u1.get_hp() > 0:
                        self.team1.team.push(u1)
                        self.team2.team.push(u2)
                    else:
                        u2.update_level()
                        self.team2.team.push(u2)
                else:
                ...
                if u1.get_hp() > 0:
                    ...
                    if u2.get_hp() > 0:
                        ...
                        u1.update_level()
                        self.team2.team.push(u1)
                else:
                    u2.update_level()
                    self.team2.team.push(u2)

        if self.team1.team.__len__() == self.team2.team.__len__() == 0:
            return "DRAW"
        elif self.team1.team.__len__() >= 1 and self.team2.team.__len__() == 0:
            return self.team1.team_name
        elif self.team2.team.__len__() >= 1 and self.team1.team.__len__() == 0:
    ...
    def optimised_mode_battle(self, criterion_team1: str, criterion_team2: str) -> str:
        ...
        # check if either of the teams are empty
        while self.team1.team.is_empty() is False and self.team2.team.is_empty() is False:

            # get first pokemon to battle from each team (last item in sorted list)
            ...
            # First check whether speed of pokemon_1 is greater than speed of pokemon_2
            if pokemon_1.get_speed() > pokemon_2.get_speed():
                # Calculate hp of pokemon_2 after attacked by pokemon_1
                pokemon_2.damage_after_attacked(pokemon_1)

                # If pokemon_2 dies (hp<=0), pokemon_2 gets deleted from the team
                # Pokemon_1 is levelled up
                if pokemon_2.get_hp() <= 0:
                    pokemon_1.update_level()
                    self.team2.team.delete_at_index(pokemon_2_index)

                # If pokemon_2 is alive after the attack, pokemon_2 attacks pokemon_1
                elif pokemon_2.get_hp() > 0:
                    # Calculate hp of pokemon_1 after attacked by pokemon_2
                    pokemon_1.damage_after_attacked(pokemon_2)

                    # If pokemon_1 dies (hp<=0), pokemon_1 get deleted from the team
                    # Pokemon_2 is levelled up
                    if pokemon_1.get_hp() <= 0:
                        pokemon_2.update_level()
                        self.team1.team.delete_at_index(pokemon_1_index)

                    # If pokemon_1 is alive after the attack, both pokemon's hp is decreased by 1. if one pokemon dies,
                    # it is deleted and the other gains one level, if both die, they are both deleted
                    else:
                        pokemon_1.decrease_hp()
                        pokemon_2.decrease_hp()
                        if pokemon_1.get_hp() <= 0 and pokemon_2.get_hp() <= 0:
            ...
            # Second check condition whether speed of pokemon_2 is larger than speed of pokemon_1:
                ...
                # If pokemon_1 dies (hp<=0), pokemon_1 gets deleted from the team
                # Pokemon_2 is levelled up
                if pokemon_1.get_hp() <= 0:
                    pokemon_2.update_level()
                    self.team1.team.delete_at_index(pokemon_1_index)

                # If pokemon_1 is alive after the attack, pokemon_1 attacks pokemon_2
                elif pokemon_1.get_hp() > 0:
                    # Calculate hp of pokemon_2 after attacked by pokemon_1
                    pokemon_2.damage_after_attacked(pokemon_1)
                    ...
                    # If pokemon_2 is alive after the attack, both pokemon's hp is decreased by 1. if one pokemon dies,
                    # it is deleted and the other gains one level, if both die, they are both deleted
                    else:
                        pokemon_1.decrease_hp()
                        pokemon_2.decrease_hp()
            ...
            # Third check condition: if the speed of pokemon_1 and pokemon_2 equals, calculate the hp after attack
            # Each pokemon hp is decreased by 1 if they both alive
            # If either of pokemons hp <= 0, that pokemon gets deleted
            elif pokemon_1.get_speed() == pokemon_2.get_speed():
                pokemon_2.damage_after_attacked(pokemon_1)
                pokemon_1.damage_after_attacked(pokemon_2)
                if pokemon_1.get_hp() > 0 and pokemon_2.get_hp() > 0:
            ...
            # reorder teams if not empty
            if self.team1.team.is_empty() is False:
                self.team1.team.poke_reorder()
            if self.team2.team.is_empty() is False:
                self.team2.team.poke_reorder()
